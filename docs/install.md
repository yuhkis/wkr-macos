...
`Y` 記号レイヤー55件と `T,` `T.` `T/` を合わせた58件は、ローマ字ではなく
...
macOS に届いた論理キーをどれだけ使っているかを数え、キーボードの絵の上にヒートマップとして
表示できます。
**既定では無効です。** 明示的に有効にしたときだけ動きます。

### 何が保存され、何が保存されないか

保存されるのは `(キーコード, Shiftの有無) → 回数` と**暦日**、その日に数えた規則表の識別子
（上流 wkr-layout の commit 名）だけです。配列を変えた前後をヒートマップで区別するためのもので、
打鍵の内容には触れません。

保存**されない**ものを明示します。

- 打鍵の順序。`WKR` と `RKW` は同じ集計になり、入力文字列は復元できません。
- 暦日より細かい時刻。「何時に何を打ったか」は残りません。
- 入力文字列、変換候補、クリップボード。
- アプリケーション名。アプリごとの内訳は取りません。

数えない条件も既存の変換ゲートに揃えてあります。**Secure Event Input が有効な間、権限が無い間、
`--exclude-app` で除外したアプリ、Apple日本語入力「ひらがな」以外の入力ソースでは1回も数えません。**
Command / Control / Option を含むショートカットと、押しっぱなしのオートリピートも数えません。
`かな` / `英数` の2キーだけは入力ソースの一致を条件から外しています（そうしないと `かな` が
...
**メニューバーのアイコンから「打鍵頻度のヒートマップを開く」を選ぶのが一番手軽です。** 常駐中の
アプリが別プロセスとしてレポートを生成し、ブラウザで開きます。集計を止めていても選べます
（過去に記録した分があるかもしれないため）。データが1件も無ければ、キーボードの絵だけが
描かれた空のページになります。

コマンドからも同じことができます。

```bash
make key-frequency-report
```
...
- **期間の切り替え**（全期間 / 直近7日 / 直近30日 / 単日）
- **JIS・US・Cornix の3つのキーボード**をタブで切り替え。同じ集計を3通りの物理配置で描きます
- `.vil` を渡した場合は **Cornix のレイヤーごとのタブ**も付きます（何か送るキーがあるレイヤーだけ）。
  macOS はどのレイヤー経由で届いたかを報告しないため、複数のレイヤーにある同じキーコードは
  同じ共有合計で、`◇` が付きます。1箇所にしかないキー（数字や記号の大半）の数値は、
  そのキーで打った回数そのものです
- tap / hold 兼用キーは**上半分が HOLD、下半分が TAP**です。2つの面は別々の回数とヒート色を持ち、
  1つの物理キーの回数として足しません
- **キートップの表示の切り替え**（刻印 / わから配列）。わから配列にすると、わから配列の規則を持つ
  27キーを**そのキーの役割**（`E` → か行、`H` → あ、`T` → □ など）で表示し、元の刻印を上に小さく
  残します（tap / hold 兼用キーの半面では `TAP · A` のように役割の行に載ります）。どのキーボードの
  タブにも、Cornix のレイヤーにも同じように効きます。色と数値は変わらず、ラベルだけが変わります。
  選んだ表示はブラウザが覚えています
- 上位キーのランキングと、指ごとの負担の集計
- その配列で**数えられないキー**の注記

### Cornix のキーマップを自分のものにする

Corne 系分割キーボードの絵は、`--vil` に Vial のエクスポートを渡すと、実際に書き込んである
各レイヤーのキートップとtap / holdの動作を描きます。数えられる動作がないレイヤーはタブにしません。

```bash
make key-frequency-report VIL=~/path/to/your-keymap.vil
```

渡さなければ組み込みの既定配列（QMK crkbd の既定に 英数 / かな を Space / Enter の隣へ足した
...
**これは実装の不足ではなく、この観測点の性質です。** キーボードが送るのは HID のキーコードで
あって、どの物理キーが押されたかではありません。

| 見えないもの | 理由 |
| --- | --- |
| 同じキーコードを複数の位置に割り当てた場合の区別 | 例えば Enter を複数のキーに割り当てると、どのキーで打っても同じキーコードが届きます。**同じ合計値を各キーに表示し、共有であることを `◇` で示します。** 頭数で割ると、存在しない精度を作ってしまうので割りません |
| 専用 LShift と `LSFT_T(...)` の hold の内訳 | どちらも同じ論理 LShift として届きます。1つの共有合計を専用キーと各HOLD面に表示しますが、どの位置から来たかは区別できません。表示箇所が増えても、全体合計・ランキング・指別集計で重複加算しません |
| `LT(...)` などレイヤーキーの hold | ファームウェア内で完結し、macOS へは届きません。数えられるのは tap 側の動作だけです |
...
`LSFT_T(...)` の hold は論理 LShift として観測できますが、物理位置の内訳は観測できないため、
「数えられない」ではなく**共有合計**として描きます。ツールチップにもその区別を表示します。
...
`LSFT_T(...)` はラップではありません。TAPでは内側のキー、HOLDでLShiftを送ります。HOLD面に
出る値は専用LShiftを含む共有論理合計であり、そのmod-tapだけをholdした回数ではありません。
