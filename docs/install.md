...
**`英数` を押したまま Enter を押すと、`thing` に置き換わります。**

1. `英数` を連打し、**画面にアルファベットが出た状態を目で確認する**（アプリによって2回でも3回でもよい）
2. 最後の `英数` を押したまま Enter を足す

wkr-macos は連打の回数を推測しません。判断するのは利用者で、wkr-macos は
自分が送ったローマ字を消して、打った物理キーの文字を入れるだけです。

`英数` を押しながらの Enter は、置き換えが起きなかった場合も前面アプリへ渡しません。
チャットアプリで意図しない送信をしないためです。

置き換えが起きない条件（design.md の 8.6）に当たると、何も起きません。Space で変換候補を
出した後、Backspace を挟んだ後、記号や小書きの後置形を打った後、12打鍵を超えた後などです。
...
| 値 | 意味 |
| --- | --- |
| `eisu+return` | `英数` を押しながら Enter（既定） |
