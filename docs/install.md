...
対象はmacOS 14以降・Apple日本語入力（ローマ字入力）・JISキーボードです。現在の配布候補はApple Silicon用で、実機での初回権限付与・変換確認は別途必要です。[確認済み範囲](verification.md)を先に確認してください。
...
Swift 6.2以降を含むXcodeを用意し、公開用branchから実行します。
...
再ビルドでad-hoc署名が変わると再許可が必要になる場合があります。見かけの許可状態と実際の権限を区別し、案内画面で再確認してください。証明書・権限の変更は端末ごとに行います。

## 4. 練習

メニュー「わから v2 を練習する」を選びます。キー位置だけを練習するなら英字入力にし、表示されたキーを順に押します。日本語入力モードの練習ではひらがなを確定後、もう一度Enterで採点します。
