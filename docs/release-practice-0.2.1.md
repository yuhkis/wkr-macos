# わから v2 練習帳 0.2.1 — Release文案

WKRを導入せずQWERTYで試すモードと、導入済みWKR v2で練習するモードを備えた独立アプリです。配列2.0.0-beta.2の教材を同梱し、オフラインで使えます。0.2.1は、ん・っ をv1と同じ N・M に戻した配列に教材の打鍵を合わせた版です（[変更点](https://github.com/yuhkis/wkr-layout/blob/v2.0.0-beta.2/docs/compatibility.md#200-beta2-での変更)）。成績の保存先は配列版ごとに分かれるため、配列2.0.0-beta.1で保存した成績は引き継がれません。入力監視・アクセシビリティの許可や変換エンジンの起動は行いません。
