...
確認状況は[検証記録](https://github.com/yuhkis/wkr-macos/blob/main/docs/verification.md)に記載します。同梱練習でのApple日本語入力の確定・採点と、権限不足時の安全停止を確認しました。古いad-hoc署名の許可登録との不一致があり、許可後の実変換・一時停止・再開は未完了です。変換確認が完了するまで配布を保留します。ダウンロード後の起動許可と物理打鍵も未確認です。Google日本語入力・azooKeyはWKRアプリの初回対応範囲に含めず、専用テーブルによる別の導入経路を案内します。Intel・他キーボード配列・全OS版・支援技術は未確認です。
