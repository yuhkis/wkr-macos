...
- **実機確認が未了である。** `T` `H` `I` `N` `G` → `英数` 連打 → `英数`+Enter で `thing` になるか、
  アプリ別に確かめる必要がある。Unicode直接注入を使うので、イベントを無視するアプリがあり得る。
- **`英数` のオートリピートが読み戻しの段を進めるかは未確認である。** 同時押しである以上、
  `英数` を押している時間は長くなる。段が進むようなら、`英数` のオートリピートだけを抑止する。
...
既定は `英数` を押しながらEnterである。`英数` は修飾キーではないので、コードはそのキー自身の
押下状態から読む。この形にすると、**読み戻しの最後の `英数` 押下がそのまま修飾キーになる**ので、
アルファベットが出た瞬間の状態でトリガできる。`英数` 自体は今までどおり素通しなので、
Apple日本語入力の段送りは利用者が見ているとおりに動く。

`英数` を押しながらのEnterは、置き換えが起きなかった場合も前面アプリへ渡さない。この組合せは
明確に意図的な操作であり、素通ししてチャットアプリでメッセージを送ってしまうより、
Enterを1回失うほうが軽いためである。

トリガは `EnglishFallbackTrigger`（`eisu+return` / `eisu+tab` / `off`）で、`--english-fallback`
またはユーザーデフォルト `EnglishFallbackTrigger` で選ぶ。フラグが優先される。設定ウィンドウは
まだないが、値の置き場所を先に決めておけば、メニューバーを作るときに束ねるだけで済む。
