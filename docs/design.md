...
## 8. 英字への打ち直し（2026-08-14提案・WKRCoreのみ実装済み）
...
- **トリガキーの選定。** `英数` は乗っ取らない。Command／Control／Optionを含む組合せは既存の
  修飾キーフィルタで弾かれるため使えない。既定はmacOSが標準で使わないファンクションキーとし、
  CLIオプションで変更できるようにする。プログラマブルキーボードならマクロキーを1つ割り当てる。
...
`WKRCore` の `EnglishFallbackJournal` として、履歴、上限、寿命、破棄条件、fail closedの判定まで
実装済みである。トランスデューサが返した `TransitionResult` を渡すだけで記録でき、Core Graphicsにも
時計にも依存しない。時刻は呼び出し側が渡す。

未実装は次の2つで、どちらもmacOS側にある。

- 履歴をコントローラへ接続すること。`英数` では変換ゲートだけを閉じ、履歴は残す必要がある（8.4）。
- トリガキーと、Backspace送出および英字列の注入。`PhysicalKey.unshiftedASCIICharacter` が
  1文字で表せるキーを返すので、`Shift+1` や `JIS-Yen` のように表せないキーだけを
  ASCII-capableキーボードレイアウトで解決する。
