...
望ましくない場面が多い**という判断から、`Y` 記号レイヤー59件を丸ごと外す選択肢を
`--symbol-layer off` として持つ。既定は `on` で、従来の挙動を変えない。

無効にしたときの `Y` は、規則を持たない他のキーと同じく素通しする。抑止しないのは
「変換対象外のキーを不必要に抑止しない」という既存方針に揃えるためであり、
**`Y` キーに何を割り当てるかは別途決める。** 素通しの `y` はApple日本語入力の
未確定文字列に入るので、Backspaceで取り消せる。

この選択肢は `Y` レイヤーだけを外す。`EY → ヶ` などUnicode出力の残り8件は同じ制約を
持つが、ローマ字で代替できないかなであり、記号とは判断が別である。
