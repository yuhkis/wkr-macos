...
AGENTS.mdは「キー列、入力本文をファイル、標準出力、解析サービスへ記録・送信しない」と定めている。
この機能は打鍵履歴をプロセス内に保持するため、次の条件つきの例外を作る。**実装前に合意が要る箇所である。**

- 保持先はプロセスのメモリだけとする。ファイル、標準出力、ログ、ネットワークへ出さない。
  ログに出るのは権限状態、モード、入力ソースID、状態コード、エラー種別、集計件数までという
  既存方針は変えない。
- 打鍵数の上限を持ち、超えたら古いものから捨てる。
- TTLを持ち、経過したら捨てる。
...
- **注意事項: 長い未確定文字列での部分確定は未確認である。** 2026-08-14の実測で
  ...
  上限を緩めるときは先に実測する。
- **AGENTS.mdの「キー列を記録しない」の記述。** 8.5の例外を実装するなら、AGENTS.mdの安全・
  プライバシー節も同じ変更で条件つきの記述へ更新する。方針の正本が実装と食い違ったままにしない。
