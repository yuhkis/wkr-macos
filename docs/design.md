...
対象は59件である。`Y` 記号レイヤー55件、`WJ → ヴ`、`T,` / `T.` / `T/` の全角約物3件。
...
`WJ → ヴ` は移せない。`vu` が返すのは `ゔ`（U+3094）で、配列が指定する `ヴ`（U+30F4）とは別の文字であり、ひらがなモードでカタカナの `ヴ` を直接出す綴りは確認できなかった。出力を `ゔ` へ変えるのは上流 `wkr-layout` の配列仕様の変更になるため、ここでは判断しない。
