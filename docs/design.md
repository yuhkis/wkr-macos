...
Space、Enter、句読点による確定はWKRが観測できるので、そこで文字数を0へ戻せば整合する。
一方、ライブ変換がキー操作なしに部分確定するのであれば、WKRに境界を知る手段はない。
この切り分けは未了で、本機能の成否を決める（8.7）。
...
- **トリガキーの選定。** `英数` は乗っ取らない。Command／Control／Optionを含む組合せは既存の
  修飾キーフィルタで弾かれるため使えない。プログラマブルキーボードでマクロキーを1つ作るのが
  最も副作用が少ない。
- **Backspaceが挟まったときの文字数の数え方。**
- **確定の境界が、WKRの観測できるキーだけで作られるか。本機能の成否を決める項目である。**
  2026-08-14の実測で、読み戻しがアルファベットへ戻すのは未確定の部分だけだと分かった。
  Space／Enter／句読点による確定であればWKRが観測できるので、そこで文字数を0へ戻せば整合する。
  ライブ変換がキー操作なしに部分確定するなら、WKRに境界は見えず、この方式は成立しない。
  確認手順は、ライブ変換を有効にしたままSpaceもEnterも句読点も押さずに長めの文を打ち続け、
  下線が途中で切れて確定するかを見る。切れるなら `英数` 連打で戻るのは末尾だけになる。
- **成立しなかった場合の選択肢は2つある。** ライブ変換の無効化を本機能の前提にするか、
  履歴の上限を部分確定が起きない長さへ抑え、超えたらfail closedにするか。後者は閾値の実測が要る。
  どちらも利用者に制約を課すため、採否は上記の切り分けの後に決める。
