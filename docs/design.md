...
**WKRが送ったローマ字そのもので長さも既知**なので、置換は決定的である。英字列は現在の
...
- **ライブ変換が進んだ状態で `英数` を連打した後の表示が、送出済みローマ字と同じ文字数か。**
  2026-08-14の実測は短い未確定文字列1件のみで、一般には未確認である。実装前に実機確認が要る。
