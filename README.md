...
- **打鍵頻度のヒートマップ**を出せます（既定オフ）。`--key-frequency on` で数え、
  ...
  **TAP** を別々に表示し、同じ論理キーを複数の動作が送る場合は共有合計を `◇` 付きで示します。
  キートップは刻印と**わから配列での役割**（`E` → か行、`H` → あ など）を切り替えられます。
...
- 打鍵頻度の記録（既定オフ）を明示的に有効にしたときだけ、キーごとの**累計回数**を暦日単位で
  ...
  必要なら `--key-frequency-retention <日数>` で指定します。配列を変えるとその時点で
  集計が切り替わり、それまでの分は `archive/` へ退避されます（削除ではありません）。
  退避した集計もヒートマップとして開けます。
