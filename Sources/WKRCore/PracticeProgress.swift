...
public struct PracticeProgress: Codable, Equatable {
    ...
    public static var empty: Self { Self(schemaVersion: 2, layoutVersion: WKRLayout.layoutVersion, lessons: [:]) }
    public static func validated(_ data: Data) -> Self? {
        guard data.count <= 16384,
              let value = try? JSONDecoder().decode(Self.self, from: data),
              value.schemaVersion == 2, value.layoutVersion == WKRLayout.layoutVersion,
