...
public struct EnglishFallbackJournal: Equatable, Sendable {
    ...
    /// Limit the in-memory journal to twelve keys. A partial IME commit is
    /// invisible to WKR, so increasing this bound requires checking the longer
    /// pre-edit before relying on its romaji count for replacement.
    ...
    private mutating func record(_ key: PhysicalKey, result: TransitionResult, at now: Double) {
        ...
        for action in result.actions {
            ...
            case let .romaji(value):
                // The relocated middle dot and question mark send IME punctuation.
                // Punctuation can commit preceding text, so connect-back must
                // never delete/rewrite across it.
                guard !value.contains("/"), !value.contains("?") else { invalidate(); return }
