...
final class StatusItemController: NSObject, NSMenuDelegate {
    ...
    var openPracticeRequested: (() -> Void)?
    var openSetupRequested: (() -> Void)?
    var pauseRequested: (() -> Void)?
    var pausedProvider: (() -> Bool)?
    ...
    override init() {
        ...
        let versionItem = NSMenuItem(title: AppVersion.display, action: nil, keyEquivalent: "")
        ...
        pauseItem.target = self; pauseItem.action = #selector(pauseChosen); menu.addItem(pauseItem)
        menu.addItem(.separator())
    ...
    func menuNeedsUpdate(_ menu: NSMenu) {
        pauseItem.title = pausedProvider?() == true ? "変換を再開" : "変換を一時停止"
    ...
    @objc private func practiceChosen() { DispatchQueue.main.async { [weak self] in self?.openPracticeRequested?() } }
    @objc private func setupChosen() { DispatchQueue.main.async { [weak self] in self?.openSetupRequested?() } }
    @objc private func pauseChosen() { DispatchQueue.main.async { [weak self] in self?.pauseRequested?() } }
