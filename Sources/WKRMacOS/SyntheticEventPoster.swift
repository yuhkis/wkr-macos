...
final class SyntheticEventPoster {
    ...
    func makeEvents(for actions: [SyntheticAction]) -> [CGEvent]? {
        var events: [CGEvent] = []
            ...
            case let .romaji(value):
                for character in value {
                    guard let keyCode = Self.keyCodes[character],
                          let pair = makeKeyPair(keyCode, shifted: character == "?")
                    else { return nil }
    ...
    private func makeKeyPair(_ keyCode: CGKeyCode, shifted: Bool = false) -> [CGEvent]? {
        ...
        keyDown.flags = shifted ? .maskShift : []
        keyUp.flags = shifted ? .maskShift : []
        mark(keyDown)
        mark(keyUp)
        return [keyDown, keyUp]
    }
