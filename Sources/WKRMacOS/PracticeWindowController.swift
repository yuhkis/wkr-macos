...
final class PracticeWindowController: NSWindowController, WKNavigationDelegate, WKScriptMessageHandler, NSWindowDelegate {
    enum LoadState: Equatable { case loading, ready, failed }
    typealias ContentRuleCompiler = (@escaping (WKContentRuleList?, Error?) -> Void) -> Void
    ...
    init?(bundle: Bundle = .main, store: PracticeProgressStore = PracticeProgressStore(),
        ...
        window.title = "わから v2 練習帳 — \(AppVersion.display)"
    ...
    func show() { NSApp.setActivationPolicy(.regular); showWindow(nil); window?.makeKeyAndOrderFront(nil); NSApp.activate() }
