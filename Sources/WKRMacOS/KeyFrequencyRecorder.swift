...
/// runs on a serial storage queue. The main-run-loop timer only snapshots
/// the in-memory tally and schedules the write.
///
...
final class KeyFrequencyRecorder {
    ...
    private let ioQueue = DispatchQueue(label: "wkr.public.frequency-storage")
    /// Accessed only on ioQueue. Failed writes stay here until a later flush.
    private var unwritten = KeyFrequencyTally()
    ...
    /// `~/Library/Application Support/io.github.yuhkis.wkr-macos.public/key-frequency.json`.
    ///
    ...
    /// Snapshot on the main queue; failures remain on ioQueue for retry.
    func flushAsync() {
        dispatchPrecondition(condition: .onQueue(.main))
        let batch = tally
        tally = KeyFrequencyTally()
        let day = currentDay()
        ioQueue.async { [self] in _ = writePending(batch, today: day) }
    }

    /// Synchronous only after the tap stops (or in isolated tests).
    @discardableResult
    func flush() -> Bool {
        let batch = tally
        tally = KeyFrequencyTally()
        let day = currentDay()
        return ioQueue.sync { writePending(batch, today: day) }
    }

    private func writePending(_ batch: KeyFrequencyTally, today: KeyFrequencyDay) -> Bool {
        unwritten.merge(batch)
        guard !unwritten.isEmpty else { return true }
        if writeBatch(unwritten, today: today) { unwritten = KeyFrequencyTally(); return true }
        return false
    }

    private func writeBatch(_ batch: KeyFrequencyTally, today: KeyFrequencyDay) -> Bool {
        ...
        case let .decoded(store):
            if KeyFrequencyRotation.isNeeded(for: store, layout: layoutIdentifier) {
                guard let base = KeyFrequencyRotation.archiveBaseName(for: store),
                      Self.moveIntoArchive(storeURL: storeURL, base: base) != nil else {
        ...
        var merged = existing
        merged.merge(batch)
        merged.prune(retainedDays: retainedDays, today: today)
        ...
        do {
            // The temporary file is private from its creation, including the
            // interval before its first write and the final atomic rename.
            let descriptor = open(temporaryURL.path, O_WRONLY | O_CREAT | O_EXCL | O_NOFOLLOW | O_CLOEXEC, S_IRUSR | S_IWUSR)
    ...
    func rotateIfLayoutChanged() -> Rotation {
        ...
        case .failed:
            // The caller disables this recorder for the launch. Conversion
            // remains available; existing data is never overwritten to proceed.
    ...
    static func rotate(storeURL: URL, layout: String, force: Bool) -> Rotation {
        ...
        case .absent:
            return .notNeeded
        case .unreadable:
            // Refuse counting until the existing file can be preserved.
            return .failed
        case .unusable:
            // Preserve an unknown/corrupt schema before starting any new tally.
