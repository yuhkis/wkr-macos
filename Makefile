...
start:
	...
	@$(MAKE) stop
	open "$(APP)" --args --input-source-id "$(INPUT_SOURCE_ID)" --input-mode-id "$(INPUT_MODE_ID)" --mode "$(MODE)" --request-permissions $(if $(filter optimistic,$(MODE)),--allow-unverified-optimistic,)
...
start-installed:
	...
	@if [ "$(MODE)" = "optimistic" ] && [ "$(ALLOW_OPTIMISTIC)" != "1" ]; then echo 'optimistic mode requires ALLOW_OPTIMISTIC=1 and a disposable untitled document.' >&2; exit 2; fi
	@$(MAKE) stop
	open "$(INSTALLED_APP)" --args --input-source-id "$(INPUT_SOURCE_ID)" --input-mode-id "$(INPUT_MODE_ID)" --mode "$(MODE)" --request-permissions $(if $(filter optimistic,$(MODE)),--allow-unverified-optimistic,)
